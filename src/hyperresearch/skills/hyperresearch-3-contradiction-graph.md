...
- `output/runs/<vault_tag>/prompt-decomposition.json` — atomic items, pipeline_tier
- All `output/runs/<vault_tag>/temp/claims-*.json` files (one per fetched note)
...
1. **Load all claims** from `output/runs/<vault_tag>/temp/claims-*.json` files.
...
5. **Write `output/runs/<vault_tag>/temp/contradiction-graph.json`** — array of ranked fight clusters.

6. **Identify consensus claims.** Claims where 3+ INDEPENDENT sources agree. Independence is computed, not assumed: run `$HPR sources independence --tag <vault_tag> -j` first — it clusters syndicated copies (same canonical URL, near-duplicate bodies, shared wire-service boilerplate) and scores members `1/cluster_size`. **Count each cluster as ONE voice** (sum of independence scores ≥ 3.0, not raw source count ≥ 3). Write consensus claims to `output/runs/<vault_tag>/temp/consensus-claims.json`. These are the "settled ground" the draft can assert confidently without hedging.

---

## Exit criterion

- `output/runs/<vault_tag>/temp/contradiction-graph.json` exists (may be empty array if corpus is univocal)
- `output/runs/<vault_tag>/temp/consensus-claims.json` exists (may be empty array)
